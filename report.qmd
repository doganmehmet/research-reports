...
You are writing this to growth (stock) investors, but sound natural as an individual writing a research report or blog. Write in smooth paragraphs for narrative flow. Only use bullet points for risks, opportunities, or portfolio recommendations. Assume the reader is financially literate; do not define basic terms. Do not start with of course, as an investment research analyst etc. Provide enough information as detailed. When mentioning a specific price or value, always include the full date it refers to (e.g., "as of 28 March 2026" or "at the end of February 2026"). Use one main title at the beginning which is catchy and describing the situation as of today, {today}. Do not use titles and subtitles, but you can use bullet points as you see fit. Use standard markdown format. Do not say this/that dataset etc.

Include charts only where they meaningfully support the discussion. Do not place charts in the introduction. You may use multiple charts per major section if they add value, but never place two charts consecutively — always have at least one paragraph of text between any two charts. Do not place any charts at the very end of the report — the report should end with text, not a chart. Use chart placeholders exactly as listed in the library below. Do not repeat the same chart within a section. Prioritize charts that strengthen the key points and insights. Always include [CHART: MSCI World Equity Index] and [CHART: Performance of Major US Stock Indices] in the report.
...
pdf_path = f"docs/market_charts_{_date.today().strftime('%Y-%m-%d')}.pdf"
with PdfPages(pdf_path) as pdf:
    for title in chart_titles:
        img_path = f"charts/{title}.png"
        try:
...
print(f"Charts PDF saved to {pdf_path}")
```
...
response = client.chat.completions.create(
    model="qwen/qwen3.7-plus",  #"xiaomi/mimo-v2-pro"
    messages=[
...
## APPENDIX: ALL GENERATED CHARTS
# The AI analysis above only embeds the charts it references. This appendix
# renders every chart generated by the report so readers can review the full
# data picture regardless of which charts the AI chose to discuss.

print("\n\n## Appendix: All Charts\n")

for chart_title, chart_path in CHART_FILENAME_LIBRARY.items():
    print(f"\n**{chart_title}**\n")
    print(f"![{chart_title}]({chart_path})\n")
```
